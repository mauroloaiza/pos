...
public class ClockInOutAction extends PosAction {
	...
	public void execute() {
		final User user = PasswordEntryDialog.getUser(Application.getPosWindow(),
				Messages.getString("ClockInOutAction.3")); //$NON-NLS-1$
		if (user == null) {
			return;
		}
	...
	private void performClockIn(User user) {
		try {
			...
			if (user.isClockedIn() != null && user.isClockedIn().booleanValue()) {
				POSMessageDialog.showMessage(Messages.getString("ClockInOutAction.11") + user.getFirstName() + " " //$NON-NLS-1$ //$NON-NLS-2$
						+ user.getLastName() + Messages.getString("ClockInOutAction.13")); //$NON-NLS-1$
				return;
			}
			...
			POSMessageDialog.showMessage(Messages.getString("ClockInOutAction.14") + user.getFirstName() + " " //$NON-NLS-1$ //$NON-NLS-2$
					+ user.getLastName() + Messages.getString("ClockInOutAction.16")); //$NON-NLS-1$
		} catch (Exception e) {
			POSMessageDialog.showError(Application.getPosWindow(), e.getMessage(), e);
		}
	}
	...
	public void performDriverOut(User user) {
		try {
			...
			POSMessageDialog
					.showMessage("Driver " + user.getFirstName() + " " + user.getLastName() + " is out for delivery."); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		} catch (Exception e) {
			POSMessageDialog.showError(Application.getPosWindow(), e.getMessage(), e);
		}
	}
	...
	public void performDriverIn(User user) {
		try {
			...
			POSMessageDialog
					.showMessage("Driver " + user.getFirstName() + " " + user.getLastName() + " is in after delivery"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		} catch (Exception e) {
			POSMessageDialog.showError(Application.getPosWindow(), e.getMessage(), e);
		}
	}

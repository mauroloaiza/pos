...
public abstract class ViewChangeAction extends PosAction {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (RootView.getInstance().getCurrentView().getViewName().equals(OrderView.VIEW_NAME)
				&& !OrderView.getInstance().getTicketView().isAllowToLogOut()) {
		...
		if (!user.hasPermission(requiredPermission)) {
			...
			// POSMessageDialog.showError(Application.getPosWindow(), "You do
			// not have permission to execute this action");
			return;
		}
		execute();
	}
		...
		if (!currentTicket.getTicketItems().isEmpty()) {
			if (hasNewItem(currentTicket)) {
				if (POSMessageDialog.showYesNoQuestionDialog(POSUtil.getFocusedWindow(),
						Messages.getString("ViewChangeAction.1"), //$NON-NLS-1$
						Messages.getString("ViewChangeAction.2")) == JOptionPane.YES_OPTION) { //$NON-NLS-1$

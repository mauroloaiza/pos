	...
	public void execute() {
		try {
			...
			message = "<html>" + //$NON-NLS-1$
					Messages.getString("RefundAction.6") + ticket.getId() + Messages.getString("RefundAction.7") //$NON-NLS-1$ //$NON-NLS-2$
			...
			double refundAmount = NumberSelectionDialog2.takeDoubleInput(message, Messages.getString("RefundAction.10"), //$NON-NLS-1$
					paidAmount);
			...
			POSMessageDialog.showMessage(
					Messages.getString("RefundAction.12") + CurrencyUtil.getCurrencySymbol() + refundAmount); //$NON-NLS-1$

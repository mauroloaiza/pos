...
public class UserListDialog extends OkCancelOptionDialog {
	...
	public void doOk() {
		...
		if (!user.isClockedIn()) {
			...
			// }
			// int option =
			// POSMessageDialog.showYesNoQuestionDialog(Application.getPosWindow(),
			// userString
			// + " not clocked in. User needs to be clocked in for drawer
			// assignment.", Messages.getString("Application.44"), "CLOCK IN",
			// "CANCEL"); //$NON-NLS-1$ //$NON-NLS-2$
			// if (option == JOptionPane.YES_OPTION) {
			// User inputUser =
			// PasswordEntryDialog.getUser(Application.getPosWindow(),
			// Messages.getString("LoginView.1"),
			// Messages.getString("LoginView.2")); //$NON-NLS-1$ //$NON-NLS-2$
			// if (inputUser == null)
			// return;
			//
			// if (user.getUserId().intValue() !=
			// inputUser.getUserId().intValue()) {
			// POSMessageDialog.showMessage(POSUtil.getFocusedWindow(), "Wrong
			// password.");
			// return;
			// }
			// Shift currentShift = ShiftUtil.getCurrentShift();
			// Calendar currentTime = Calendar.getInstance();
			// user.doClockIn(Application.getInstance().getTerminal(),

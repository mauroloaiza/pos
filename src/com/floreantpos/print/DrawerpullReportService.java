...
public class DrawerpullReportService {
	...
	public static DrawerPullReport buildDrawerPullReport() throws Exception {
		...
		try {
			...
			// Boolean.FALSE));
			// criteria.add(Restrictions.eq(GiftCertificateTransaction.PROP_TERMINAL,
			// terminal));
	...
	private static void populateVoidSection(Session session, Terminal terminal, DrawerPullReport report) {
		...
		for (Iterator iter = list.iterator(); iter.hasNext();) {
			Ticket ticket = (Ticket) iter.next();
			/*
			 * DrawerPullVoidTicketEntry entry = new
			 * DrawerPullVoidTicketEntry(); entry.setCode(ticket.getId());
			 * entry.setAmount(ticket.getSubtotalAmount());
			 * entry.setReason(ticket.getVoidReason()); if (ticket.isWasted()) {
			 * entry.setHast("Yes"); //$NON-NLS-1$ } else { entry.setHast("No");
			 * //$NON-NLS-1$ }
			 * 

	...
	public static void printDrawerPullReport(DrawerPullReport drawerPullReport, Terminal terminal) {
		...
		try {
			...
			if (drawerPullReport.getAssignedUser() != null)
				parameters.put("user", //$NON-NLS-1$
						Messages.getString("PosPrintService.4") + drawerPullReport.getAssignedUser().getFullName()); //$NON-NLS-1$
			...
			JRBeanCollectionDataSource dataSource = new JRBeanCollectionDataSource(
					Arrays.asList(new DrawerPullReport[] { drawerPullReport }));
	...
	public static void printServerTipsReport(TipsCashoutReport report) {
		...
		try {
			...
			JRDataSource dataSource = new JRTableModelDataSource(new TipsCashoutReportTableModel(report.getDatas(),
					new String[] { "ticketId", "saleType", "ticketTotal", "tips" })); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
			JasperPrint jasperPrint = JasperFillManager.fillReport(mainReport, parameters, dataSource);
			jasperPrint.setProperty(ReceiptPrintService.PROP_PRINTER_NAME,
					Application.getPrinters().getReportPrinter());

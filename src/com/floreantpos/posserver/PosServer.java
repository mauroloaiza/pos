...
public class PosServer implements Runnable {
	...
	static void listen(ServerSocket ss) throws Exception {
		...
		// String posdft = "", pamt = "", tamt = "", cardtype = "", server = "",
		// table = "", check = "", edc = "";
		// String Card[] = { "DEBIT", "MCRD", "VISA", "DINER", "DSCVR", "AMEX",
		// "JCB", "PL", "CASH", "GIFT" };
		...
		while (true) {
			...
			// JAXBContext.newInstance(POSRequest.class);
			...
			//
			// /*User user =
			// UserDAO.getInstance().findUserBySecretKey(posRequest.posDefaultInfo.server);
			// List<Ticket> ticketsForUser =
			// TicketDAO.getInstance().findOpenTicketsForUser(user);
			//
			...
			//
			// JAXBContext messageContext =
			// JAXBContext.newInstance(POSResponse.class);
			...
			// resp = resp.replaceAll("<\\?xml version=\"1.0\"
	...
	public static void main(String[] args) throws Exception {
		 ...
		 * POSResponse posResponse = new POSResponse();
		 * 
		 * Ident ident = new Ident(); ident.setId("345"); ident.setTtype("45");
		 * 
		 * POSDefaultInfo posDefaultInfo = new POSDefaultInfo();
		 * posDefaultInfo.setServer("1111"); posDefaultInfo.setRes("1");
		 * posDefaultInfo.setrText("Success");
		 * 
		 * posResponse.setIdent(ident);
		 * posResponse.setPosDefaultInfo(posDefaultInfo);
		 ...
		 * checks.setCheckList(new ArrayList<Check>());
		 * 
		 * for (Ticket ticket : ticketsForUser) { List<Integer> tableNumbers =
		 * ticket.getTableNumbers(); if (tableNumbers != null &&
		 * tableNumbers.size() > 0) { Check chk = new Check();
		 * chk.setTableNo(String.valueOf(tableNumbers.get(0)));
		 * chk.setTableName("-"); chk.setChkName("-");
		 * chk.setChkNo(String.valueOf(ticket.getId()));
		 * chk.setAmt(String.valueOf(Math.round((ticket.getDueAmount() -
		 * ticket.getTaxAmount()) * 100)));
		 ...
		 * 
		 * JAXBContext messageContext =
		 * JAXBContext.newInstance(POSResponse.class); Marshaller marshaller =
		 * messageContext.createMarshaller(); final StringWriter dataWriter =
		 * new StringWriter(); marshaller.marshal(posResponse, dataWriter);
		 * 
		 * String string = dataWriter.toString(); string = string.replaceAll(

	...
	public void actionPerformed(ActionEvent e) {
		try {
			...
			if (TEST.equalsIgnoreCase(command)) {
				...
				boolean createDatabase = DatabaseUtil.createDatabase(connectionString2, hibernateDialect, driverClass,
						user, pass, generateSampleData);
				this.setCursor(Cursor.getDefaultCursor());

				if (createDatabase) {
					// JOptionPane.showMessageDialog(DatabaseConfigurationView.this,
					// Messages.getString("DatabaseConfigurationDialog.35"));
					// //$NON-NLS-1$
					JOptionPane.showMessageDialog(POSUtil.getBackOfficeWindow(),

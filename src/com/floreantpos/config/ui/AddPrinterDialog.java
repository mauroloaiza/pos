...
public class AddPrinterDialog extends POSDialog {
	...
	public void initUI() {
		...
		// cbDevice.setModel(new
		// DefaultComboBoxModel(PrintServiceLookup.lookupPrintServices(null,
		// null)));
	...
	protected void doAddPrinter() {
		/*
		 * VirtualPrinter vp = (VirtualPrinter)
		 * cbVirtualPrinter.getSelectedItem(); if (vp == null) {
		 * POSMessageDialog.showError(this,
		 * Messages.getString("AddPrinterDialog.18")); //$NON-NLS-1$ return; }
		 */
		...
		if (selectedObject instanceof PrintService) {
			 ...
			 * Messages.getString("AddPrinterDialog.19")); //$NON-NLS-1$ return;
			 */

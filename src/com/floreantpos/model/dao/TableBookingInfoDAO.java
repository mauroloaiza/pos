...
public class TableBookingInfoDAO extends BaseTableBookingInfoDAO {
	...
	public Collection<ShopTable> getBookedTables(Date startDate, Date endDate) {
		...
		try {
			...
			// criteria.add(Restrictions.ge(TableBookingInfo.PROP_TO_DATE,
			// startDate));
	...
	public void setBookingStatus(TableBookingInfo bookingInfo, String bookingStatus, List<ShopTable> tables) {
		...
		try {
			...
			if (bookingStatus.equals(TableBookingInfo.STATUS_SEAT)
					|| bookingStatus.equals(TableBookingInfo.STATUS_DELAY)) {
				ShopTableDAO.getInstance().bookedTables(tables);
			}

			if (bookingStatus.equals(TableBookingInfo.STATUS_CANCEL)
					|| bookingStatus.equals(TableBookingInfo.STATUS_NO_APR)
	...
	/*
	 * public void closeBooking(TableBookingInfo bookingInfo, List<ShopTable>
	 * tables) {
	 * 
	 * Session session = null; Transaction tx = null;
	 * 
	 * try { session = createNewSession(); tx = session.beginTransaction();
	 * 
	 ...
	 * LogFactory.getLog(TableBookingInfo.class).error(e); throw new
	 * RuntimeException(e); } finally { closeSession(session); }
	 * 
	 * }
	 */

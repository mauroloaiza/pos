...
public class SalesSummaryDAO extends _RootDAO {
	...
	public SalesStatistics findKeyStatistics(Date start, Date end, UserType userType, Terminal terminal) {
		...
		try {
			...
			{
				// determine number of guests within the specified time. this
...

			{
				...
				for (Object object : shifts) {
					...
					List<OrderType> values = Application.getInstance().getOrderTypes(); // change
																						// enum
	...
	private void findRecordByProfitCenter(Date start, Date end, UserType userType, Terminal terminal, Session session,
			SalesStatistics salesSummary, Shift shift, OrderType ticketType) {
		Criteria criteria;

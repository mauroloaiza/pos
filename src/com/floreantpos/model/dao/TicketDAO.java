...
public class TicketDAO extends BaseTicketDAO {
	...
	// public void saveTransaction(Ticket ticket,
	// com.floreantpos.model.PosTransaction transaction, Terminal terminal, User
	// user) throws Exception {
	// Session session = null;
	// Transaction tx = null;
	// try {
	// if(transaction instanceof CashTransaction) {
	// terminal.setCurrentBalance(terminal.getCurrentBalance() +
	...
	private void adjustInventoryItems(Session session, Ticket ticket) {
		...
		for (TicketItem ticketItem : ticketItems) {
			...
			for (RecepieItem recepieItem : recepieItems) {
				...
				inventoryItem.setTotalPackages(
						inventoryItem.getTotalPackages() - (itemQuantity / inventoryItem.getUnitPerPackage()));

...
public class OrderTypeDAO extends BaseOrderTypeDAO {
	...
	public boolean containsOrderTypeObj() {
		...
		try {
			session = createNewSession();
			Query query = session
					.createSQLQuery("select count(s.MENU_ITEM_ID), count(s.ORDER_TYPE_ID) from ITEM_ORDER_TYPE s");
	...
	/*
	 * This method is used for updating menu item order type name to order type
	 * object.
	 * 
	 */
	public void updateMenuItemOrderType() {
		...
		try {
			...
			Query query = session.createSQLQuery(
					"Update ITEM_ORDER_TYPE t SET t.ORDER_TYPE_ID=(Select o.id from ORDER_TYPE o where o.NAME=t.ORDER_TYPE)");
			query.executeUpdate();
			tx.commit();
		} catch (Exception e) {
			tx.rollback();

...
public class GratuityDAO extends BaseGratuityDAO {
	...
	public List<Gratuity> findByUser(User user) throws PosException {
		...
		try {
			...
			throw new PosException(
					Messages.getString("GratuityDAO.0") + user.getFirstName() + " " + user.getLastName()); //$NON-NLS-1$ //$NON-NLS-2$
		} finally {
	...
	public TipsCashoutReport createReport(Date fromDate, Date toDate, User user) {
		...
		try {
			...
			// criteria.add(Restrictions.eq(Ticket.PROP_DRAWER_RESETTED,
			// Boolean.FALSE));
			...
			throw new PosException(Messages.getString("GratuityDAO.4") + user.getFirstName() + " " + user.getLastName(), //$NON-NLS-1$ //$NON-NLS-2$
					e);
		} finally {
			closeSession(session);
		}

...
public abstract class _BaseRootDAO {
		...
		if (null == configFile) {
			if (null == sessionFactory)
				throw new RuntimeException(
						"The session factory has not been initialized (or an error occured during initialization)"); //$NON-NLS-1$
			else
				return sessionFactory;
		} else {
			if (null == sessionFactoryMap)
				throw new RuntimeException("The session factory for '" + configFile //$NON-NLS-1$
						+ "' has not been initialized (or an error occured during initialization)"); //$NON-NLS-1$
			else {
	...
	public class TransactionPointer {
		 ...
		 *             if the timeout has been reached before transaction
		 *             completion
		 * @throws Throwable

...
public class TicketItem extends BaseTicketItem implements ITicketItem {
	...
	// public TicketItemModifierGroup findTicketItemModifierGroup(MenuModifier
	// menuModifier, boolean createNew) {
	// MenuItemModifierGroup menuItemModifierGroup =
	// menuModifier.getMenuItemModifierGroup();
	//
	...
	// if
	// (ticketItemModifierGroup.getMenuItemModifierGroup().getId().equals(menuItemModifierGroup.getId()))
	// {
	...
	// }
	//
	// TicketItemModifierGroup ticketItemModifierGroup = new
	// TicketItemModifierGroup();
	...
	// }
	//
	// public TicketItemModifierGroup findTicketItemModifierGroup(int
	// menuModifierGroupId) {
	// List<TicketItemModifierGroup> ticketItemModifierGroups =
	// getTicketItemModifierGroups();
	//
	...
	// for (TicketItemModifierGroup ticketItemModifierGroup :
	// ticketItemModifierGroups) {
	// if (ticketItemModifierGroup.getMenuItemModifierGroup().getId() ==
	// menuModifierGroupId) {
	// return ticketItemModifierGroup;
	// }
	// }
	...
	public TicketItemModifier convertToAddOn(MenuModifier menuModifier) {
		...
		ticketItemModifier.setModifierType(TicketItemModifier.EXTRA_MODIFIER);
		// ticketItemModifier.setTaxRate(menuModifier.getTax() == null ? 0 :
		// menuModifier.getTax().getRate());
	...
	public boolean isMergable(TicketItem otherItem, boolean merge) {
		if (this.isFractionalUnit() || this.getItemId() == 0
				|| (this.getCookingInstructions() != null && this.getCookingInstructions().size() > 0)) {
			return false;
		}
		if (!this.isHasModifiers() && !otherItem.isHasModifiers()) {
			if (this.isTreatAsSeat() == otherItem.isTreatAsSeat()
					&& this.getSeatNumber().intValue() == otherItem.getSeatNumber().intValue()) {
				return true;
			}
		...
		// List<TicketItemModifierGroup> thisModifierGroups =
		// this.getTicketItemModifierGroups();
		// List<TicketItemModifierGroup> thatModifierGroups =
		// otherItem.getTicketItemModifierGroups();
		...
		// }
		//
		// Comparator<TicketItemModifierGroup> comparator = new
		// Comparator<TicketItemModifierGroup>() {
		// @Override
		// public int compare(TicketItemModifierGroup o1,
		// TicketItemModifierGroup o2) {
		// return o1.getMenuItemModifierGroup().getId() -
		// o2.getMenuItemModifierGroup().getId();
		// }
		// };
		//
		...
		//
		// Iterator<TicketItemModifierGroup> thisIterator =
		// thisModifierGroups.iterator();
		// Iterator<TicketItemModifierGroup> thatIterator =
		// thatModifierGroups.iterator();
		//
	...
	public boolean isMergableModifiers(List<TicketItemModifier> thisModifiers, List<TicketItemModifier> thatModifiers,
			boolean merge) {
	...
	public TicketItemModifier findTicketItemModifierFor(MenuModifier menuModifier, Multiplier multiplier) {
		...
		for (TicketItemModifier ticketItemModifier : modifiers) {
			Integer itemId = ticketItemModifier.getModifierId();
			if (itemId != null && itemId.intValue() == menuModifier.getId().intValue()
					&& multiplier.getName().equals(ticketItemModifier.getMultiplierName())) {
				return ticketItemModifier;
			}
		}
		return null;
	...
	public TicketItemModifier findTicketItemModifierFor(MenuModifier menuModifier, String sectionName,
		...
		for (TicketItemModifier ticketItemModifier : modifiers) {
			Integer itemId = ticketItemModifier.getModifierId();
			if (multiplier != null) {
				if ((itemId != null && itemId.intValue() == menuModifier.getId().intValue()) && (sectionName != null
						&& sectionName.equals(ticketItemModifier.getSectionName()) && (multiplier != null
								&& multiplier.getName().equals(ticketItemModifier.getMultiplierName())))) {
					return ticketItemModifier;
				}
			}

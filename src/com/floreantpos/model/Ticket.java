...
public class Ticket extends BaseTicket {
	...
	public Date getDeliveryDate() {
		...
		/*
		 * if (deliveryDate == null) { deliveryDate = getCreateDate(); Calendar
		 * c = Calendar.getInstance(); c.add(Calendar.MINUTE, 10); deliveryDate
		 * = c.getTime(); }
		 */
	...
	// List<TicketCouponAndDiscount> couponAndDiscounts =
	...
	public double calculateServiceCharge() {
		/*
		 * if (getType() != OrderType.DINE_IN) { return 0; }
		 */
	...
	public PosTransaction getBartabTransaction() {
		...
		for (PosTransaction transaction : getTransactions()) {
			if (transaction instanceof CreditCardTransaction && transaction.isAuthorizable()
					&& !transaction.isCaptured() && !transaction.isVoided()) {
				return transaction;
			}
		}
		for (PosTransaction transaction : getTransactions()) {

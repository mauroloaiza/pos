...
public class KitchenTicket extends BaseKitchenTicket {
	...
	public OrderType getType() {
		...
		/*
		 * if (StringUtils.isEmpty(type)) { return OrderType.DINE_IN; }
		 */
	...
	// newticketItems.addAll(((Ticket)
	// SerializationUtils.clone(ticket)).getTicketItems());
	//
	// Map<String, List<TicketItem>> itemMap = new LinkedHashMap<String,
	// List<TicketItem>>();
	//
	...
	// continue;
	// }
	// List<TicketItem> itemListInMap =
	// itemMap.get(newItem.getItemId().toString());
	//
	...
	// List<TicketItem> itemFromMap =
	// itemMap.get(originalTicketItem.getItemId().toString());
	//
	// if (itemFromMap != null) {
	...
	private static void includeModifiers(TicketItem ticketItem, KitchenTicket kitchenTicket) {
		...
		if (ticketItemModifiers != null) {
			for (TicketItemModifier itemModifier : ticketItemModifiers) {
				/*
				 * if (!itemModifier.isShouldPrintToKitchen()) { continue; }
				 */
		...
		if (addOns != null) {
			for (TicketItemModifier ticketItemModifier : addOns) {
				/*
				 * if (!ticketItemModifier.isShouldPrintToKitchen()) { continue;
				 * }
				 */

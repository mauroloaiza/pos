...
public class ReceiptPrintService {
	...
	public static JasperPrint createJasperPrint(JasperReport report, Map<String, String> properties,
	...
	/*
	 * public static void printTicket(Ticket ticket) { try {
	 * 
	 * TicketPrintProperties printProperties = new TicketPrintProperties(
	 * "*** ORDER " + ticket.getId() + " ***", false, true, true); //$NON-NLS-1$
	 * //$NON-NLS-2$ printProperties.setPrintCookingInstructions(false); HashMap
	 ...
	 * jasperPrint.setProperty(PROP_PRINTER_NAME,
	...
	public static void printTicket(Ticket ticket, boolean printTipsBlock) {
		try {
			...
			TicketPrintProperties printProperties = new TicketPrintProperties("*** ORDER " + ticket.getId() + " ***", //$NON-NLS-1$ //$NON-NLS-2$
					false, true, true);
			...
			for (TerminalPrinters terminalPrinters2 : terminalPrinters) {
				...
				if (printerType == VirtualPrinter.RECEIPT) {
					...
					Printer printer = new Printer(terminalPrinters2.getVirtualPrinter(),
							terminalPrinters2.getPrinterName());
	...
	public static void printTransaction(PosTransaction transaction) {
		try {
			...
			TicketPrintProperties printProperties = new TicketPrintProperties(
					Messages.getString("ReceiptPrintService.3"), true, true, true); //$NON-NLS-1$
			printProperties.setPrintCookingInstructions(false);
	...
	public static void printTransaction(PosTransaction transaction, boolean printStoreCopy, boolean printCustomerCopy) {
		try {
			...
			TicketPrintProperties printProperties = new TicketPrintProperties(
					Messages.getString("ReceiptPrintService.6"), true, true, true); //$NON-NLS-1$
			printProperties.setPrintCookingInstructions(false);
			HashMap map = populateTicketProperties(ticket, printProperties, transaction);
		...
		if (printProperties.isShowFooter()) {
			...
			if (transaction != null) {
				...
				if (transaction.isCard()) {
					...
					String cardInformationForReceipt = CardConfig.getPaymentGateway().getProcessor()
							.getCardInformationForReceipt(transaction);
	...
	private static void populateRefundProperties(Set<PosTransaction> transactions, HashMap map) {
		...
		map.put("additionalProperties", "<html><b>" + Messages.getString("ReceiptPrintService.1") + " "
				+ CurrencyUtil.getCurrencySymbol() + "&nbsp;" + refundAmount + "</b></html>");
	}
	...
	private static StringBuilder buildTicketHeader(Ticket ticket, TicketPrintProperties printProperties) {
		...
		addColumn(ticketHeaderBuilder,
				POSConstants.RECEIPT_REPORT_TERMINAL_LABEL + Application.getInstance().getTerminal().getId());
		endRow(ticketHeaderBuilder);
	...
	private static StringBuilder buildMultiCurrencyTotalAmount(Ticket ticket, TicketPrintProperties printProperties) {
		...
		// addColumn(currencyAmountBuilder, getHtmlText("Paid", 10,
		// CENTER));//$NON-NLS-1$
		...
		for (Currency currency : CurrencyUtil.getAllCurrency()) {
			...
			addColumn(currencyAmountBuilder,
					getHtmlText(decimalFormat.format(ticket.getTotalAmount() * rate), 12, RIGHT));
			// addColumn(currencyAmountBuilder,
			// getHtmlText(decimalFormat.format(ticket.getPaidAmount() * rate),
			// 10, RIGHT));
			addColumn(currencyAmountBuilder,
					getHtmlText(decimalFormat.format(ticket.getDueAmount() * rate), 12, RIGHT));
			endRow(currencyAmountBuilder);
			rowCount++;
		}
	...
	public static JasperPrint createKitchenPrint(String virtualPrinterName, KitchenTicket ticket, String deviceName)
			throws Exception {
		HashMap map = new HashMap();
		...
		map.put(SHOW_HEADER_SEPARATOR, Boolean.TRUE);
		map.put(CHECK_NO,
				POSConstants.RECEIPT_REPORT_TICKET_NO_LABEL + ticket.getTicketId() + "-" + ticket.getSequenceNumber());
		if (ticket.getTableNumbers() != null && ticket.getTableNumbers().size() > 0) {
			map.put(TABLE_NO, POSConstants.RECEIPT_REPORT_TABLE_NO_LABEL + ticket.getTableNumbers());
		}
		...
		map.put("printerName", "Printer Name : " + virtualPrinterName); //$NON-NLS-1$ //$NON-NLS-2$
																		// //$NON-NLS-3$
	...
	public static void printToKitchen(Ticket ticket) {
		...
		try {
			...
			for (KitchenTicket kitchenTicket : kitchenTickets) {
				...
				JasperPrint jasperPrint = createKitchenPrint(printer.getVirtualPrinter().getName(), kitchenTicket,
						deviceName);

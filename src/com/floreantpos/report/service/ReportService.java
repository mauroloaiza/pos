...
public class ReportService {
	...
	public ServerProductivityReport getServerProductivityReport(Date fromDate, Date toDate) {
		...
		try {
			...
			// criteria.add(Restrictions.eq(User.PROP_USER_TYPE,
			// User.USER_TYPE_SERVER));
	...
	public SalesBalanceReport getSalesBalanceReport(Date fromDate, Date toDate, User user) {
		...
		try {
			...
			report.setCashReceiptsAmount(
					calculateCreditReceipt(session, CashTransaction.class, fromDate, toDate, user));
			report.setCreditCardReceiptsAmount(
					calculateCreditReceipt(session, CreditCardTransaction.class, fromDate, toDate, user));

			// report.setGiftCertSalesAmount(calculateGiftCertSoldAmount(session,
			// fromDate, toDate));
			// report.setGiftCertReturnAmount(calculateCreditReceipt(session,
			// GiftCertificateTransaction.class, fromDate, toDate));

			//
			//// gift cert
			// criteria =
			// session.createCriteria(GiftCertificateTransaction.class);
			// criteria.createAlias(GiftCertificateTransaction.PROP_TICKET,
			// "t");
			// criteria.add(Restrictions.ge("t." + Ticket.PROP_CREATE_DATE,
			// fromDate));
			// criteria.add(Restrictions.le("t." + Ticket.PROP_CREATE_DATE,
			// toDate));
			// criteria.add(Restrictions.eq("t." + Ticket.PROP_VOIDED,
			// Boolean.FALSE));
			// criteria.add(Restrictions.eq("t." + Ticket.PROP_REFUNDED,
			// Boolean.FALSE));
			// projectionList = Projections.projectionList();
			...
			// visa card summery
			report.setVisaCreditCardAmount(
					calculateVisaCreditCardSummery(session, CreditCardTransaction.class, fromDate, toDate, user));
			report.setMasterCardAmount(
					calculateMasterCardSummery(session, CreditCardTransaction.class, fromDate, toDate, user));
			report.setAmexAmount(calculateAmexSummery(session, CreditCardTransaction.class, fromDate, toDate, user));
			report.setDiscoveryAmount(
					calculateDiscoverySummery(session, CreditCardTransaction.class, fromDate, toDate, user));
	...
	private double calculateVisaCreditCardSummery(Session session, Class transactionClass, Date fromDate, Date toDate,
			User user) {
		// cash receipt
		Criteria criteria = session.createCriteria(transactionClass);
		criteria.add(Restrictions.ge(PosTransaction.PROP_TRANSACTION_TIME, fromDate));
	...
	private double calculateMasterCardSummery(Session session, Class transactionClass, Date fromDate, Date toDate,
			User user) {
		// cash receipt
		Criteria criteria = session.createCriteria(transactionClass);
		criteria.add(Restrictions.ge(PosTransaction.PROP_TRANSACTION_TIME, fromDate));
	...
	private double calculateAmexSummery(Session session, Class transactionClass, Date fromDate, Date toDate,
			User user) {
		// cash receipt
		Criteria criteria = session.createCriteria(transactionClass);
		criteria.add(Restrictions.ge(PosTransaction.PROP_TRANSACTION_TIME, fromDate));
	...
	private double calculateDiscoverySummery(Session session, Class transactionClass, Date fromDate, Date toDate,
			User user) {
		// cash receipt
		Criteria criteria = session.createCriteria(transactionClass);
		criteria.add(Restrictions.ge(PosTransaction.PROP_TRANSACTION_TIME, fromDate));

...
public class SalesReport extends Report {
	...
	public void refresh() throws Exception {
		...
		map.put("dateRange", //$NON-NLS-1$
				ReportService.formatShortDate(getStartDate()) + " to " + ReportService.formatShortDate(getEndDate())); //$NON-NLS-1$
		...
		map.put("currency", Messages.getString("SalesReport.8") + CurrencyUtil.getCurrencyName() + " (" //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
				+ CurrencyUtil.getCurrencySymbol() + ")"); //$NON-NLS-1$
	...
	public void createModels() {
		...
		List<Ticket> tickets = TicketDAO.getInstance().findTickets(date1, date2,
				getReportType() == Report.REPORT_TYPE_1 ? true : false, getTerminal());
			...
			for (TicketItem ticketItem : ticketItems) {
				...
				if (modifiers != null) {
					for (TicketItemModifier modifier : modifiers) {
						...
						key += "-" + modifier.getName() + modifier.getModifierType() + "-" + modifier.getUnitPrice() //$NON-NLS-1$ //$NON-NLS-2$
								+ modifier.getTaxRate();
						...
						modifierReportItem
								.setGrossTotal(modifierReportItem.getGrossTotal() + modifier.getTotalAmount());

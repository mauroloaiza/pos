...
public class SalesExceptionReport {
	...
	public void addVoidToVoidData(Ticket ticket) {
		 ...
		 * voidData.setAmount(voidData.getAmount() + amount); found = true;
		 * break; } }
		 */
		// if (!found) {
	...
	public class DiscountTableModel extends ListTableModel {
		public DiscountTableModel() {
			setColumnNames(new String[] { "no", "name", "code", "totalCount", "totalDiscount", "totalNetSales", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$
					"totalGuests", "partySize", "checkSize", "countPercent", "ratioDnet" }); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
		}

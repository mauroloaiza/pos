...
public class DejavooProxyServer implements HttpHandler {
	...
	private void sendTicketsByServer(String serverId) throws Exception {
		...
		List<Ticket> ticketList = TicketDAO.getInstance().findTicketsForUser(PaymentStatusFilter.OPEN, POSConstants.ALL,
				user);
	...
	private StringBuilder createInvoiceList(List<Ticket> ticketList) {
		...
		for (Ticket ticket : ticketList) {
			...
			String name = ticket.getCustomer() == null ? ticket.getOwner().getFirstName()
					: ticket.getCustomer().getName();
			String invoiceFormat = String.format(invoice, ticket.getId(), name,
					numberFormat.format(ticket.getDueAmount() * 100), ticket.isClosed() ? "closed" : "open");
	...
	private void sendTicketDetail(String ticketId) throws Exception {
		...
		builder.append(
				String.format("<InvoiceData id=\"%s\" name=\"%s\">", ticket.getId(), ticket.getOwner().getFirstName()));
		...
		if (transactions != null) {
			...
			for (PosTransaction posTransaction : transactions) {
				...
				builder.append(String.format(
						"<Payment " + "refId=\"%s\" " + "name=\"%s\" " + "amount=\"%s\" " + "tip=\"%s\" "
								+ "type=\"%s\" />",
						// + "acctLast4=\"%s\"/>",
						posTransaction.getId(), paymentType, numberFormat.format(posTransaction.getAmount() * 100),
						numberFormat.format(posTransaction.getTipsAmount() * 100),
	...
	private void sendData(String data) throws Exception {
		URL url = new URL(
				"http://spinpos.net:80/spin/cgi.html?TerminalTransaction=" + URLEncoder.encode(data, "utf-8"));

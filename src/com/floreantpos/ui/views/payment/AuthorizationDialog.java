...
class AuthorizationDialog extends POSDialog implements Runnable {
		...
		for (Iterator iterator = transactions.iterator(); iterator.hasNext();) {
			...
			try {
				...
				if (cardReader == CardReader.EXTERNAL_TERMINAL) {
					...
					txtStatus.append(Messages.getString("AuthorizationDialog.1") + transaction.getId() //$NON-NLS-1$
							+ Messages.getString("AuthorizationDialog.2")); //$NON-NLS-1$
					continue;
				}
				...
				txtStatus.append(Messages.getString("AuthorizationDialog.1") + transaction.getId() //$NON-NLS-1$
						+ Messages.getString("AuthorizationDialog.4") + e.getMessage() + "\n"); //$NON-NLS-1$ //$NON-NLS-2$
			}
		}
	...
	private void initComponents() {
		...
		JLabel labelGateway = new JLabel(
				Messages.getString("PaymentProcessWaitDialog.8") + CardConfig.getPaymentGateway().getProductName()); //$NON-NLS-1$
		...
		scrollPane
				.setBorder(BorderFactory.createCompoundBorder(new EmptyBorder(20, 30, 20, 30), scrollPane.getBorder()));

...
public class SettleTicketProcessor implements CardInputListener {
	...
	public static void showTransactionCompleteMsg(final double dueAmount, final double tenderedAmount, Ticket ticket,
	...
	public void doVoidBartab(PosTransaction barTabTransaction) throws Exception, PosException {
		...
		CardProcessor cardProcessor = CardConfig.getPaymentGateway().getProcessor();
		// String question = "Do you want to void pre authorized card
		// transaction?";
		// int option =
		// POSMessageDialog.showYesNoQuestionDialog(POSUtil.getFocusedWindow(),
		// question, "Void transaction");
		//
		// if (option == JOptionPane.YES_OPTION) {
		cardProcessor.voidTransaction(barTabTransaction);
		// }
		// else {
		// throw new PosException("Pre authorized transaction must be captured
		// or voided first");
		// }
	}

	public void doAfterSettleTask(PosTransaction transaction, final double dueAmount, boolean printTicket)
		...
		if (ticket.getDueAmount() > 0.0) {
			int option = JOptionPane.showConfirmDialog(Application.getPosWindow(), POSConstants.CONFIRM_PARTIAL_PAYMENT,
	...
	public void cardInputted(CardInputProcessor inputter, PaymentType selectedPaymentType) {
		...
		try {
			...
			POSMessageDialog.showError(POSUtil.getFocusedWindow(),
					"Could not make payment. Please try again or consult system administrator.", exception);
		} finally {

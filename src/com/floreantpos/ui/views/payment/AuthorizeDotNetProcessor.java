...
public class AuthorizeDotNetProcessor implements CardProcessor {
	public void preAuth(PosTransaction transaction) throws Exception {
		...
		if (result.isApproved()) {
			...
			throw new Exception(Messages.getString("AuthorizeDotNetProcessor.2") //$NON-NLS-1$
					+ result.getResponseReasonCodes().get(0).getReasonText());
		} else {
			throw new Exception(Messages.getString("AuthorizeDotNetProcessor.3") //$NON-NLS-1$
					+ result.getResponseReasonCodes().get(0).getReasonText());
		}
	}
	...
	public void captureAuthAmount(PosTransaction transaction) throws Exception {
		...
		// Create transaction
		Transaction authCaptureTransaction = merchant.createAIMTransaction(TransactionType.PRIOR_AUTH_CAPTURE,
				new BigDecimal(transaction.getAmount()));
		...
		if (result.isApproved()) {
			...
			throw new Exception(Messages.getString("AuthorizeDotNetProcessor.6") //$NON-NLS-1$
					+ result.getResponseReasonCodes().get(0).getReasonText());
		} else {
			throw new Exception(Messages.getString("AuthorizeDotNetProcessor.7") //$NON-NLS-1$
					+ result.getResponseReasonCodes().get(0).getReasonText());
		}
	}
	...
	public void captureNewAmount(PosTransaction transaction) throws Exception {
		...
		if (result.isApproved()) {
			...
			throw new Exception(Messages.getString("AuthorizeDotNetProcessor.8") //$NON-NLS-1$
					+ result.getResponseReasonCodes().get(0).getReasonText());
		} else {
			throw new Exception(Messages.getString("AuthorizeDotNetProcessor.9") //$NON-NLS-1$
					+ result.getResponseReasonCodes().get(0).getReasonText());
		}
	}
	...
	public void voidAmount(PosTransaction transaction) throws Exception {
		...
		if (result.isApproved()) {
			...
			throw new Exception(Messages.getString("AuthorizeDotNetProcessor.10") //$NON-NLS-1$
					+ result.getResponseReasonCodes().get(0).getReasonText());
		} else {
			throw new Exception(Messages.getString("AuthorizeDotNetProcessor.11") //$NON-NLS-1$
					+ result.getResponseReasonCodes().get(0).getReasonText());
		}
	}
	...
	// merchant.createAIMTransaction(TransactionType.VOID, new
	// BigDecimal(amount));
	...
	// }
	// else if (result.isDeclined()) {
	// throw new Exception(Messages.getString("AuthorizeDotNetProcessor.12") +
	// result.getResponseReasonCodes().get(0).getReasonText()); //$NON-NLS-1$
	// }
	// else {
	// throw new Exception(Messages.getString("AuthorizeDotNetProcessor.13") +
	// result.getResponseReasonCodes().get(0).getReasonText()); //$NON-NLS-1$
	// }
	// }
	...
	private CreditCard createCard(PosTransaction transaction) {
		...
		if (StringUtils.isNotEmpty(transaction.getCardTrack())) {
			...
			return createCard(transaction.getCardNumber(), transaction.getCardExpMonth(), transaction.getCardExpYear(),
	...
	public static void main(String[] args) throws Exception {
		...
		//
		// Merchant merchant = Merchant.createMerchant(environment, apiLoginID,
		// transactionKey);
		// merchant.setDeviceType(net.authorize.DeviceType.VIRTUAL_TERMINAL);
		// merchant.setMarketType(net.authorize.MarketType.RETAIL);
		// //merchant.setMD5Value(MD5Value);
		...
		// merchant.createAIMTransaction(TransactionType.AUTH_ONLY, new
		// BigDecimal(100));
		// authTransaction.setCreditCard(creditCard);
		//
		// Result<Transaction> result = (Result<Transaction>)
		// merchant.postTransaction(authTransaction);
		//
		// if (result.isApproved()) {
		//
		...
		//
		// Transaction authCaptureTransaction =
		// merchant.createAIMTransaction(TransactionType.PRIOR_AUTH_CAPTURE, new
		// BigDecimal(100));
		...
		final BankCardMagneticTrack track = BankCardMagneticTrack.from(
				"%B4111111111111111^SHAH/RIAR^1803101000000000020000831000000?;4111111111111111=1803101000020000831?"); //$NON-NLS-1$

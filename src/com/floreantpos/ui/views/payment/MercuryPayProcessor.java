...
public class MercuryPayProcessor implements CardProcessor {
	...
	private String doPreAuth(Ticket ticket, String cardTrack, double amount) throws IOException, Exception {
		String xml = StreamUtils
				.toString(MercuryPayProcessor.class.getResourceAsStream("/com/mercurypay/ws/sdk/mercuryAuth.xml")); //$NON-NLS-1$
		...
		mpswr.addParameter("tran", xml); // Set WebServices 'tran' //$NON-NLS-1$
											// parameter to the XML transaction
											// request
		mpswr.addParameter("pw", CardConfig.getMerchantPass()); // Set //$NON-NLS-1$
																// merchant's
																// WebServices
																// password
		mpswr.setWebMethodName("CreditTransaction"); // Set //$NON-NLS-1$
														// WebServices webmethod
														// to selected type
	...
	public void captureAuthAmount(PosTransaction transaction) throws Exception {
		String xml = StreamUtils.toString(
				MercuryPayProcessor.class.getResourceAsStream("/com/mercurypay/ws/sdk/mercuryPreAuthCapture.xml")); //$NON-NLS-1$
		Ticket ticket = transaction.getTicket();

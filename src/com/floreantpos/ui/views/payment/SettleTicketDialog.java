...
//TODO: REVISE CODE
public class SettleTicketDialog extends POSDialog
		implements PaymentListener, TicketViewerTableChangeListener, RefreshableView {
	...
	// Messages.getString("SettleTicketDialog.18")); //$NON-NLS-1$
	// return;
	// }
	//
	// try {
	// String loyaltyid =
	// JOptionPane.showInputDialog(Messages.getString("SettleTicketDialog.19"));
	// //$NON-NLS-1$
	//
	...
	// Messages.getString("SettleTicketDialog.21")); //$NON-NLS-1$
	// paymentView.updateView();
	// } catch (Exception e) {
	// POSMessageDialog.showError(Application.getPosWindow(),
	// Messages.getString("SettleTicketDialog.22"), e); //$NON-NLS-1$
	// }
	// }
	...
	public String buildLoyaltyApiURL(Ticket ticket, String loyaltyid) {
		...
		transactionURL += "&server=" //$NON-NLS-1$
				+ POSUtil.encodeURLString(ticket.getOwner().getFirstName() + " " + ticket.getOwner().getLastName()); //$NON-NLS-1$
	...
	public void settleBartab() throws Exception {
		...
		int option2 = POSMessageDialog.showYesNoQuestionDialog(this, "This will void Pre-Authorized amount in Tab",
				"Warning", "Void", "Cancel");

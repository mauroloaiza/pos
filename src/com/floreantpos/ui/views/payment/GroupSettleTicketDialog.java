...
public class GroupSettleTicketDialog extends POSDialog implements CardInputListener {
	...
	private JPanel createTicketInfoPanel() {
		...
		JPanel ticketInfoPanel = new com.floreantpos.swing.TransparentPanel(
				new MigLayout("wrap 2,fill, hidemode 3", "[][grow]", "")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	...
	public void doGroupSettle(PaymentType paymentType) {
		try {
			 ...
			 * doSettleBarTabTicket(ticket); return; }
			 */
	...
	public void showTransactionCompleteMsg(final double dueAmount, final double tenderedAmount, List<Ticket> ticket,
	...
	private void settleTicket(CardProcessor cardProcessor, PosTransaction transaction) {
		try {
			...
			for (Ticket ticket : tickets) {
				...
				if (ticket.getOrderType().isPreAuthCreditCard()) {// authorize
																	// onlly do
																	// not
																	// capture
	...
	private void setTransactionAmounts(PosTransaction transaction) {
		if (tickets.get(tickets.size() - 1).getId() == transaction.getTicket().getId()) {
			if (totalTenderAmount > totalDueAmount) {
				transaction
						.setTenderAmount(totalTenderAmount - totalDueAmount + transaction.getTicket().getDueAmount());

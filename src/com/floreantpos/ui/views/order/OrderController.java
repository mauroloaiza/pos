...
public class OrderController
	...
	public void payOrderSelected(Ticket ticket) {
		...
		// String message = Messages.getString("OrderController.0") +
		// ticket.getId() + Messages.getString("OrderController.1");
		// //$NON-NLS-1$ //$NON-NLS-2$
		// if (ticket.isPaid()) {
		// message = Messages.getString("OrderController.2") + ticket.getId() +
		// Messages.getString("OrderController.3"); //$NON-NLS-1$ //$NON-NLS-2$
		// }
		//
	...
	public static void openModifierDialog(ITicketItem ticketItemObject) {
		try {
			...
			for (TicketItemModifier ticketItemModifier : addedTicketItemModifiers) {
				 ...
				 * ticketItemModifier.setUnitPrice(ticketItemModifier.
				 * getUnitPrice() * defaultSellPortion / 100); }
				 */

 ...
 * @author MShahriar
 */
public class ModifierSelectionDialog extends POSDialog
	...
	private void doFinishModifierSelection() {
		List<MenuItemModifierGroup> menuItemModiferGroups = modifierSelectionModel.getMenuItem()
	...
	public void modifierSelected(MenuModifier modifier, Multiplier multiplier) {
		...
		if (numModifiers >= maxQuantity) {
			...
			// return;
			// if
			// (Application.getInstance().getRestaurant().isAllowModifierMaxExceed())
			// {
			...
			for (Iterator iterator = menuItemModiferGroups.iterator(); iterator.hasNext();) {
				...
				if (!ticketItem.requiredModifiersAdded(ticketItemModifierGroup) && hasModifiers) {
					...
					POSMessageDialog.showMessage(POSUtil.getFocusedWindow(),
							"Please select minimum quantity of each group!");
					return;
				}
				...
				// POSMessageDialog.showMessage(POSUtil.getFocusedWindow(),
				// "Please select quantity below the maximum quantity of each
				// group!");
				// return;
				// }
				// else {

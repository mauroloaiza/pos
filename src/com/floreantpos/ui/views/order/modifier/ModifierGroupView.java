...
public class ModifierGroupView extends JPanel implements ComponentListener {
	...
	public ModifierGroupView(ModifierSelectionModel modifierSelectionModel) {
		...
		simpleScrollPane.getScrollUpButton()
				.setPreferredSize(new Dimension(100, TerminalConfig.getTouchScreenButtonHeight()));
		simpleScrollPane.getScrollDownButton()
				.setPreferredSize(new Dimension(100, TerminalConfig.getTouchScreenButtonHeight()));
	...
	private class ModifierGroupButton extends POSToggleButton implements ActionListener {
		...
		private void updateButtonText() {
			...
			// "</center></body></html>";//$NON-NLS-1$ //$NON-NLS-2$
			// }
			// else {
			string = "<html><body><center> " + menuModifierGroup.getDisplayName() + "<br/>"
					+ "<strong><span style='color:white;background-color:orange;margin:0;" + "'>&nbsp; " //$NON-NLS-1$ //$NON-NLS-2$
					+ menuModifierGroup.getMenuItemModifierGroup().getMinQuantity()
	...
	public void componentResized(ComponentEvent e) {
		int verticalUnitIncrement = simpleScrollPane.getViewport().getVisibleRect().height
				- TerminalConfig.getTouchScreenButtonHeight();

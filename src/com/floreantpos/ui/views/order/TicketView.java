...
public class TicketView extends JPanel {
	...
	private void createItemSearchPanel() {
		...
		itemSearchPanel.add(btnSearch,
				"grow, span, width " + PosUIManager.getSize(60) + "!, height " + PosUIManager.getSize(40) + "!");
	}
	...
	private void createTicketItemControlPanel() {
		...
		// ticketItemActionPanel.setLayout(new MigLayout("fill, wrap 1, ins 0",
		// "[fill, grow 100, shrink 100]", ""));
	...
	public void updateView() {
		...
		if (selectedItem != null) {
			if (TerminalConfig.isActiveCustomerDisplay()) {
				String sendMessageToCustomerDisplay = getDisplayMessage(selectedItem,
						ticket.getTotalAmount().toString());
				DrawerUtil.setItemDisplay(TerminalConfig.getCustomerDisplayPort(), sendMessageToCustomerDisplay);
			}
		}

		btnTotal.setText(POSConstants.TOTAL.toUpperCase() + " " + CurrencyUtil.getCurrencySymbol()
				+ NumberUtil.formatNumber(ticket.getTotalAmount()));
		/*
		 * if (ticket.getTotalAmount() > 0) { //btnTotal.setText(
		 * "<html><h2>Total " + Application.getCurrencySymbol() +
		 * NumberUtil.formatNumber(ticket.getTotalAmount()) + "</h2></html>");
		 * btnTotal.setText("Total " +Application.getCurrencySymbol() +
		 * NumberUtil.formatNumber(ticket.getTotalAmount())); } else {
		...
		if (ticket.getId() == null) {
			...
			titledBorder.setTitle(ticket.getTicketType() + " " + Messages.getString("TicketView.37") + ticket.getId()
					+ " Table# " + getTableNumbers(ticket.getTableNumbers()));

			/*
			 * titledBorder.setTitle(ticket.getTicketType() + " Ticket ["+
			 * ticket.getId()+"]," + "Table [" +
			 * getTableNumbers(ticket.getTableNumbers())+"]"); //$NON-NLS-1$
			 * //$NON-NLS-2$
			 */}
	...
	private void showCustomerDisplayInfo(Object selected) {
		if (TerminalConfig.isActiveCustomerDisplay()) {
			...
			if (ticketItem != null) {
				String sendMessageToCustomerDisplay = getDisplayMessage(ticketItem,
						NumberUtil.formatNumber(ticket.getTotalAmount(), true));
				DrawerUtil.setItemDisplay(TerminalConfig.getCustomerDisplayPort(), sendMessageToCustomerDisplay);
			}
		}
	}

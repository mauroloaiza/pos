...
public class PizzaModifierView extends JPanel implements ModifierGroupSelectionListener {
	...
	public PizzaModifierView(TicketItem ticketItem, MenuItem menuItem,
			PizzaModifierSelectionDialog pizzaModifierSelectionDialog) {
	...
	public void updateView() {
		...
		JScrollPane js = new JScrollPane(groupPanel, JScrollPane.VERTICAL_SCROLLBAR_NEVER,
		...
		for (MenuModifier menuModifier : modifiers) {
			...
			groupPanel.getContentPane().add(new ModifierButton(menuModifier, selectedMultiplier,
					pizzaModifierSelectionDialog.getSelectedSize()));
		}
	...
	private class ModifierButton extends PosButton implements ActionListener {
		...
		public ModifierButton(MenuModifier modifier, Multiplier multiplier, MenuItemSize menuItemSize) {
			...
			setText("<html><center>" + modifier.getDisplayName() + "<br/>" //$NON-NLS-1$ //$NON-NLS-2$
					+ modifier.getPriceForSizeAndMultiplier(menuItemSize, true, multiplier) + "</center></html>");

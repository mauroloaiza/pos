...
public class PizzaModifierSelectionDialog extends POSDialog implements ModifierSelectionListener, ActionListener {
	...
	private void initComponents() {
		...
		pizzaItemActionPanel.setLayout(
				new MigLayout("fillx,ins 2 0 2 0", "[" + size + "px][grow][" + size + "px]", "[" + size + "]"));
		...
		for (Section section : sectionList) {
			for (Iterator iterator = ticketItemModifiers.iterator(); iterator.hasNext();) {
				...
				if (!ticketItemModifier.isInfoOnly()
						&& section.getSectionName().equals(ticketItemModifier.getSectionName())) {
	...
	private boolean doFinishModifierSelection() {
		...
		for (Iterator iterator = menuItemModiferGroups.iterator(); iterator.hasNext();) {
			...
			if (!ticketItem.requiredModifiersAdded(menuItemModifierGroup)) {
				POSMessageDialog.showMessage(POSUtil.getFocusedWindow(),
						String.format("Required modifiers for group %s not added!",
		...
		if (!editMode) {
			...
			int showYesNoQuestionDialog = POSMessageDialog.showYesNoQuestionDialog(POSUtil.getFocusedWindow(),
					"Do you want to create more pizza?", "More Pizza");
	...
	public void modifierSelected(MenuModifier modifier, Multiplier multiplier) {
		...
		if (isMaxModifierAddedFromGroup(menuItemModifierGroup, countModifier)) {
			POSMessageDialog.showError("You have added maximum number of allowed modifiers from group "
		...
		TicketItemModifier findTicketItemModifierForWholeSection = ticketItem.findTicketItemModifierFor(modifier,
				getMainSection().getSectionName(), null);
		TicketItemModifier ticketItemModifier = ticketItem.findTicketItemModifierFor(modifier,
				selectedSection.getSectionName(), null);
		...
		if (!selectedSection.mainSection) {
			...
			for (Section sec : sectionList) {
				...
				if (sec.getSectionName().startsWith("Half")) {
					...
					TicketItemModifier ticItemModifier = ticketItem.findTicketItemModifierFor(modifier,
							sec.getSectionName(), null);
					if (ticItemModifier != null && ticketItemModifier == null) {
						if (findTicketItemModifierForWholeSection != null) {
							POSMessageDialog.showMessage(POSUtil.getFocusedWindow(),
									"Item already added in WHOLE section!");
							return;
						}
		...
		////// ----------------------------------------
		if (findTicketItemModifierForWholeSection != null && !selectedSection.mainSection
				&& ticketItemModifier == null) {
			int questionDialog = POSMessageDialog.showYesNoQuestionDialog(POSUtil.getFocusedWindow(),
		...
		if (ticketItemModifier == null || ticketItemModifier.isPrintedToKitchen() || !itemSizeSame) {
			...
			// ticketItemModifier.setItemCount(ticketItemModifier.getItemCount()
			// + 1);
	...
	private TicketItemModifier convertToTicketItemModifier(TicketItem ticketItem, MenuModifier menuModifier,
			OrderType type, Multiplier multiplier) {
		TicketItemModifier ticketItemModifier = new TicketItemModifier();
	...
	private class Section extends JPanel implements MouseListener {
		...
		public void clearItems() {
			...
			if (isPrintedModifierExist) {
				POSMessageDialog.showMessage(POSUtil.getFocusedWindow(),
						"Modifiers that sent to kitchen can not be deleted!");
			}
		...
		public void clearItem(TicketItemModifier ticketItemModifier,
				SectionModifierTableModel sectionModifierTableModel) {
			boolean isPrintedModifierExist = false;
			if (sectionModifierTableModel.getRows() == null) {
				return;
			}
	...
	public class Pizza extends JPanel {
		...
		public void paintComponent(Graphics g) {
			...
			g2d.fill(circle);
			// drawCircleByCenter(g, getWidth() / 2, getHeight() / 2, width /
			// 2);
			//
			// g.setColor(new Color(255, 251, 211));
	...
	class SizeAndCrustSelectionPane extends JPanel {
		...
		public SizeAndCrustSelectionPane() {
			...
			crustPanel
					.setBorder(BorderFactory.createTitledBorder(null, "CRUST", TitledBorder.CENTER, TitledBorder.TOP));
		...
		protected void renderCrusts(MenuItemSize size) {
			...
			for (final PizzaPrice pizzaPrice : availablePrices) {
				...
				crustButton.setText(
						"<html><center>" + pizzaPrice.getCrust().getName() + "<br/>" + CurrencyUtil.getCurrencySymbol()
	...
	public class ModifierTableCellRenderer extends PosTableRenderer {
		...
		// MultiLineTableCellRenderer multiLineTableCellRenderer = new
	...
	// rendererComponent.setForeground(Color.BLACK);
	// }
	// // String text = "<html><body style= \"width:100%;\"><table
	// width=\"100%\" border=\"0\" style=\" table-layout : fixed;\"><tr><td
	...
	// </p>*/
	//
	// String text = "<html><div style=\"width:100%; border:1;\"><p style=\"
	// text-align:center; \">" + ticketItemModifier.getName() + "<span> "
	// + ticketItemModifier.getUnitPrice() + "</span></p></div></html>";
	//
	// // String text =ticketItemModifier.getName()+"
	// "+ticketItemModifier.getUnitPrice();
	// rendererComponent.setText(text);
	// }
	//
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand() == "FULL") {
			...
			if (ticketItem.getTicketItemModifiers() != null && ticketItem.getTicketItemModifiers().size() > 0) {
				int questionDialog = POSMessageDialog.showYesNoQuestionDialog(POSUtil.getFocusedWindow(),
						"Items of the section will be deleted, Are you sure to change section mode?",
						"Change Section Mode");
				if (questionDialog != JOptionPane.YES_OPTION) {

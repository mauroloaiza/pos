...
public class DefaultOrderServiceExtension extends OrderServiceExtension {
	...
	public boolean finishOrder(int ticketId) {
		...
		// POSMessageDialog.showError(Application.getPosWindow(), "Please select
		// tickets of type HOME DELIVERY or PICKUP or DRIVE THRU");
		// return false;
		// }
		...
		if (due != 0) {
			POSMessageDialog.showError(Application.getPosWindow(),
					Messages.getString("DefaultOrderServiceExtension.2")); //$NON-NLS-1$
			return false;
		}

		int option = JOptionPane.showOptionDialog(Application.getPosWindow(),
				Messages.getString("DefaultOrderServiceExtension.3") + ticket.getId() //$NON-NLS-1$
						+ Messages.getString("DefaultOrderServiceExtension.4"), //$NON-NLS-1$
				Messages.getString("DefaultOrderServiceExtension.5"), //$NON-NLS-1$

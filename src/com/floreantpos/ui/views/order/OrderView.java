...
public class OrderView extends ViewPanel implements PaymentListener, TicketEditListener, RefreshableView {
	...
	private void handleTicketItemSelection() {
		...
		if (selectedItem == null) {
			...
			// btnAddOn.setEnabled(selectedTicketItem != null &&
			// selectedTicketItem.isHasModifiers());
		}
		// btnVoid.setEnabled(item.canAddAdOn());
		// btnAddOn.setEnabled(item.canVoid());

		// else if (selectedObject instanceof TicketItemModifier) {
		// selectedTicketItem = ((TicketItemModifier)
		// selectedObject).getParent().getParent();
		...
		// orderView.getCategoryView().setSelectedCategory(menuCategory);
		//
		// TicketItemModifier ticketItemModifier = (TicketItemModifier)
		// selectedObject;
	...
	private JPanel createTicketSummeryPanel() {
		...
		// !ticket.isCustomerWillPickup()) {
		// ticketAmountPanel.add(lblDeliveryCharge, "newline,growx,aligny
		// center"); //$NON-NLS-1$
	...
	protected void doAddEditCustomer() {
		...
		if (!dialog.isCanceled()) {
			...
			btnCustomer.setText("<html><body><center>CUSTOMER<br>\"" + dialog.getSelectedCustomer().getName()
					+ "\"</center></body></html>");
	...
	protected void addDiscount() {
		 ...
		 * Messages.getString("TicketView.39"),
		 * Messages.getString("TicketView.40"), ticketItem.getDiscountAmount());
		 * //$NON-NLS-1$ //$NON-NLS-2$ if (Double.isNaN(d)) { return; }
		 * 
		 * ticketItem.setDiscountAmount(d);
		 * ticketView.getTicketViewerTable().repaint(); ticketView.updateView();
		 */
	}
	...
	// if (!(object instanceof TicketItem)) {
	// POSMessageDialog.showError(Application.getPosWindow(),
	// Messages.getString("TicketView.20")); //$NON-NLS-1$
	// return;
	// }
	//
	// TicketItem ticketItem = (TicketItem) object;
	...
	// ModifierSelectionDialog dialog = new ModifierSelectionDialog(new
	// ModifierSelectionModel(ticketItem, menuItem), true);
	// dialog.open();
	// ticketView.updateView();
	// }
	...
	public void paymentCanceled() {
		...
		if (currentTicket.getId() != null && currentTicket.getDueAmount() > 0.0) {
			POSMessageDialog.showError(POSUtil.getFocusedWindow(),
					"Payment is not fully completed, ticket can not be cancelled!");
			return;
		}

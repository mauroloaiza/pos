...
public class SplitTicketDialog extends POSDialog {
	...
	public void saveTicket(TicketForSplitView view, Session session) {
		...
		if (originalTicket.getId() != splitTicket.getId()
				&& originalTicket.getPaidAmount() > originalTicket.getTotalAmount()) {
	...
	private void transferTicketTransactionToSplitTicket(Ticket originalTicket, Ticket splitTicket) {
		...
		try {
			for (Iterator iterator = transactions.iterator(); iterator.hasNext();) {
				...
				if (NumberUtil.roundToTwoDigit(posTransaction.getAmount()) == NumberUtil
						.roundToTwoDigit(splitTicket.getDueAmount())) {
			...
			newTransaction.setAmount(
					remainingAmount > splitTicket.getDueAmount() ? splitTicket.getDueAmount() : remainingAmount);

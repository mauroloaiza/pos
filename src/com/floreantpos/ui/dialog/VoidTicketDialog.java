...
public class VoidTicketDialog extends POSDialog {
	...
	public VoidTicketDialog() {
		...
		try {
			...
			POSMessageDialog.showError(Application.getPosWindow(),
					com.floreantpos.POSConstants.CANNOT_LOAD_VOID_REASONS, e);
		}
	...
	private void initComponents() {
		...
		org.jdesktop.layout.GroupLayout transparentPanel2Layout = new org.jdesktop.layout.GroupLayout(
				transparentPanel2);
		transparentPanel2.setLayout(transparentPanel2Layout);
		transparentPanel2Layout.setHorizontalGroup(transparentPanel2Layout
				...
				.add(transparentPanel2Layout.createSequentialGroup().add(jLabel1)
						...
						.add(transparentPanel2Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
								.add(chkItemsWasted).add(org.jdesktop.layout.GroupLayout.TRAILING,
										transparentPanel2Layout.createSequentialGroup()
						...
						.addContainerGap()));
		transparentPanel2Layout.setVerticalGroup(transparentPanel2Layout
				.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
				.add(transparentPanel2Layout.createSequentialGroup()
						.addContainerGap().add(transparentPanel2Layout
								.createParallelGroup(
	...
	private void btnVoidActionPerformed(java.awt.event.ActionEvent evt) {
		try {
			...
			if (ticket.getPaidAmount() > 0) {
				...
				refundAmount = NumberSelectionDialog2.takeDoubleInput("Enter refund amount",
				...
				if (tipsAmount > 0) {
					if (POSMessageDialog.showYesNoQuestionDialog(POSUtil.getFocusedWindow(),
							"Do you want to refund tips?", "Confirm") == JOptionPane.YES_OPTION) {
				...
				if (refundAmount > paidAmount) {
					POSMessageDialog.showMessage(POSUtil.getFocusedWindow(),
							"Refund amount cannot be greater than paid amount.");
					return;
				}
			...
			// save the action
			ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), ActionHistory.VOID_CHECK,
					com.floreantpos.POSConstants.RECEIPT_REPORT_TICKET_NO_LABEL + ":" + ticket.getId() + "; Total" //$NON-NLS-1$ //$NON-NLS-2$
							+ ": " + NumberUtil.formatNumber(ticket.getTotalAmount())); //$NON-NLS-1$

...
public class PasswordEntryDialog extends OkCancelOptionDialog implements ActionListener {
	...
	private JPanel createKeyboardPanel() {
		...
		String[][] numbers = { { "7", "8", "9" }, { "4", "5", "6" }, { "1", "2", "3" }, { "0" } }; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$ //$NON-NLS-8$ //$NON-NLS-9$ //$NON-NLS-10$
																									// //$NON-NLS-11$
																									// //$NON-NLS-12$
	...
	public void doOk() {
		...
		// if (!validPassword) {
		// //POSMessageDialog.showError(this, "The password is not valid.
		// Password can only contain digit.");
		//
		// return;
		// }
		//
	...
	/*
	 * private synchronized boolean checkLogin(String secretKey) { user =
	 * UserDAO.getInstance().findUserBySecretKey(secretKey); if(user == null) {
	...
	private synchronized boolean checkLogin(String secretKey) {
		...
		if (isAutoLogOffMode()) {
			...
			if (viewName.equals(TableMapView.VIEW_NAME)) {
				 ...
				 * else if (viewName.equals(SwitchboardView.VIEW_NAME)) { if
				 * (!user.hasPermission(UserPermission.VIEW_ALL_OPEN_TICKETS)) {
				 * statusLabel.setText(
				 * "user has no permission to access this view"); return false;
				 * } }
				 */
			...
			/*
			 * if (TerminalConfig.getDefaultView().equals(OrderType.DINE_IN.
			 * toString())) { if
			 * (!user.hasPermission(UserPermission.CREATE_TICKET)) {
			 * statusLabel.setText("user has no permission to access this view"
			 * ); return false; } } else if
			 * (TerminalConfig.getDefaultView().equals(OrderType.TAKE_OUT.
			 * toString())) { if
			 * (!user.hasPermission(UserPermission.CREATE_TICKET)) {
			 * statusLabel.setText("user has no permission to access this view"
			 * ); return false; } }else if
			 * (TerminalConfig.getDefaultView().equals(OrderType.RETAIL.toString
			 * ())) { if (!user.hasPermission(UserPermission.CREATE_TICKET)) {
			 * statusLabel.setText("user has no permission to access this view"
			 * ); return false; } }else if
			 * (TerminalConfig.getDefaultView().equals(OrderType.FOR_HERE.
			 * toString())) { if
			 * (!user.hasPermission(UserPermission.CREATE_TICKET)) {
			 * statusLabel.setText("user has no permission to access this view"
			 * ); return false; } } else
	...
	/**
	 * @param autoLogOffMode
	 *            the autoLogOffMode to set
	 */

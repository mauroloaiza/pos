...
public class DefaultTableSelectionView extends TableSelector implements ActionListener {
	...
	private void init() {
		...
		JideScrollPane scrollPane = new JideScrollPane(buttonsPanel, JideScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
	...
	public synchronized void redererTables() {
		...
		for (ShopTable shopTable : tables) {
			...
			tableButton.setFont(
					new Font(tableButton.getFont().getName(), Font.BOLD, PosUIManager.getTableNumberFontSize()));
	...
	private void tableSelected(ActionEvent e) {
		...
		if (shopTable == null) {
			POSMessageDialog.showError(this,
					Messages.getString("TableSelectionDialog.2") + e + Messages.getString("TableSelectionDialog.3")); //$NON-NLS-1$ //$NON-NLS-2$
			return;
		}
	...
	private void doReleaseTable(ShopTable shopTable) {
		if (shopTable.isServing()) {
			int option = POSMessageDialog.showYesNoQuestionDialog(POSUtil.getFocusedWindow(),
					Messages.getString("DefaultTableSelectionView.8"), //$NON-NLS-1$
					Messages.getString("DefaultTableSelectionView.9")); //$NON-NLS-1$
	...
	private void checkTables() {
		...
		int result = POSMessageDialog.showYesNoQuestionDialog(Application.getPosWindow(),
				Messages.getString("TableSelectionView.0"), //$NON-NLS-1$

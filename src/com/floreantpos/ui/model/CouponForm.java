...
public class CouponForm extends BeanEditor implements ItemListener {
	...
	public void itemStateChanged(ItemEvent event) {
		 ...
		 * cbListItems.setModel(menuGroups); } else if (event.getItem() ==
		 * Discount.COUPON_QUALIFICATION_NAMES[2]) { List<MenuCategory>
		 * menuCategories = MenuCategoryDAO.getInstance().findAll();
	...
	protected void updateView() {
		...
		/*
		 * else if (coupon.getQUALIFICATION_TYPE() ==
		 * Discount.QUALIFICATION_TYPE_GROUP) {
		 * cbListItems.selectItems(coupon.getMenuGroups()); } else if
		 * (coupon.getQUALIFICATION_TYPE() ==
		 * Discount.QUALIFICATION_TYPE_CATEGORY) {
	...
	protected boolean updateModel() {
		 ...
		 * coupon.setMenuGroups(cbListItems.getCheckedValues()); } else if
		 * (qualificationType == Discount.QUALIFICATION_TYPE_CATEGORY) {

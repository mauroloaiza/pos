...
public class PizzaModifierForm extends BeanEditor {
	...
	private void initComponents() {
		...
		// lelfInputPanel.add(chkUseFixedPrice, "skip 1,aligny top");
		// //$NON-NLS-1$
	...
	private void createPizzaModifierPricePanel(JPanel generalTabPanel) {
		...
		pizzaModifierPriceTableModel = new PizzaPriceTableModel(pizzaModifierPriceList,
				MultiplierDAO.getInstance().findAll());
		pizzaModifierPriceTable.setModel(pizzaModifierPriceTableModel);

		/*
		 * DefaultCellEditor cellEditor = new DefaultCellEditor(new
		 * DoubleTextField()); cellEditor.setClickCountToStart(1);
		 * 
		 * for (int i = 0; i < pizzaModifierPriceTable.getColumnCount(); i++) {
		 * pizzaModifierPriceTable.setDefaultEditor(pizzaModifierPriceTable.
		 * getColumnClass(i), cellEditor); }
		 */

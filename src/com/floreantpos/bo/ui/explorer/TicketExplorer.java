...
public class TicketExplorer extends TransparentPanel {
		...
		btnVoid.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (index < 0) {
						POSMessageDialog.showMessage(POSUtil.getBackOfficeWindow(),
								POSConstants.SELECT_ONE_TICKET_TO_VOID);
						return;
					}
					...
					if (POSMessageDialog.showYesNoQuestionDialog(TicketExplorer.this, POSConstants.CONFIRM_DELETE,
		...
		btnVoidAll.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (POSMessageDialog.showYesNoQuestionDialog(TicketExplorer.this, POSConstants.CONFIRM_DELETE,
							POSConstants.DELETE_ALL) != JOptionPane.YES_OPTION) {
						return;
					}
	...
	class TicketExplorerTableModel extends ListTableModel<Ticket> {
		String[] columnNames = { POSConstants.ID, POSConstants.CREATED_BY.toUpperCase(),
				POSConstants.CREATE_TIME.toUpperCase(), POSConstants.SETTLE_TIME.toUpperCase(),
				POSConstants.SUBTOTAL.toUpperCase(), POSConstants.DISCOUNT.toUpperCase(),

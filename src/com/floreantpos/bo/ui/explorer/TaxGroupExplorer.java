		...
		btn_newGroup.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (inventoryGroup != null) {
					...
					noRootTreeTableModel.insertNodeInto(new DefaultMutableTreeTableNode(inventoryGroup), root,
							root.getChildCount());
				}
		...
		deleteButton.addActionListener(new ActionListener() {
			...
			private void deleteInventoryTax() {
				try {
					...
					DefaultMutableTreeTableNode lastPathComponent = (DefaultMutableTreeTableNode) treePath
					...
					if (((DefaultMutableTreeTableNode) lastPathComponent).getUserObject() instanceof TaxGroup) {
						...
						if (POSMessageDialog.showYesNoQuestionDialog(getRootPane(), POSConstants.CONFIRM_DELETE,
						...
						MutableTreeTableNode tableNode = findTreeNodeForTax(
								(MutableTreeTableNode) noRootTreeTableModel.getRoot(), attributeGroup.getId());
						...
						MutableTreeTableNode tableNode = findTreeNodeForTax(
								(MutableTreeTableNode) noRootTreeTableModel.getRoot(), String.valueOf(tax.getId()));
						if (tableNode.getParent() != null) {
							noRootTreeTableModel.removeNodeFromParent(tableNode);
						}
	...
	private void editSelectedRow() {
		try {
			...
			if (((DefaultMutableTreeTableNode) lastPathComponent).getUserObject() instanceof TaxGroup) {
				...
				String groupName = JOptionPane.showInputDialog(POSUtil.getFocusedWindow(), "Group name",

...
public class DefaultCustomerListView extends CustomerSelector {
	...
	public void initUI() {
		...
		setBorder(new TitledBorder(null, POSConstants.SELECT_CUSTOMER.toUpperCase(), TitledBorder.CENTER,
	...
	protected void doSearchCustomer() {
		try {
			...
			if (StringUtils.isEmpty(mobile) && StringUtils.isEmpty(loyalty) && StringUtils.isEmpty(name)) {
				...
				customerListTableModel
						.setNumRows(CustomerDAO.getInstance().getNumberOfCustomers(mobile, loyalty, name));

...
public class ShopTableForm extends BeanEditor<ShopTable> {
	...
	public ShopTableForm() {
		...
		statusPanel.setBorder(new TitledBorder(null, Messages.getString("ShopTableForm.4"), TitledBorder.LEADING, //$NON-NLS-1$
		...
		final FloorLayoutPlugin floorLayoutPlugin = (FloorLayoutPlugin) ExtensionManager
		...
		if (floorLayoutPlugin != null) {
			...
			btnCreateType.addActionListener(new ActionListener() {
				...
				@Override
				public void actionPerformed(ActionEvent e) {
					BeanEditorDialog dialog = new BeanEditorDialog(POSUtil.getBackOfficeWindow(),
							floorLayoutPlugin.getBeanEditor());
					dialog.open();
					tableTypeCBoxList.setModel(ShopTableTypeDAO.getInstance().findAll());
				}
			});
	...
	public boolean delete() {
		try {
			...
			int option = POSMessageDialog.showYesNoQuestionDialog(POSUtil.getBackOfficeWindow(),
					Messages.getString("ShopTableForm.14"), //$NON-NLS-1$
	...
	public boolean deleteAllTables() {
		...
		int option = POSMessageDialog.showYesNoQuestionDialog(POSUtil.getBackOfficeWindow(),
				Messages.getString("ShopTableForm.20"), //$NON-NLS-1$

...
public class MercuryWebRequest {
	...
	public void addParameter(String paramName, String paramValue) throws Exception {
		...
		if (paramName.equals("tran"))
			paramValue = paramValue.replace("<", "&lt;").replace(">", "&gt;").replace("\t", "").replace("\n", "")
					.replace("\r", ""); // Remove less-than, greater-than,
	...
	public String sendRequest() throws Exception {
		...
		rd.close();
		// Parse correct value from SOAP response according to HTTP response
		// code
		int start = 0, end = 0;
		if (httpResponseCode != 200) {
			error = true;
	 ...
	 * 
	 * @throws Exception
	 *             When missing required parameters or containing invalid
	 *             parameters
	 */
	private void validateRequiredParameters() throws Exception {
		...
		if (!mWSParameters.containsKey("pw"))
			throw new Exception(
					String.format(mMPSExceptionString, "WebServices password parameter (\"pw\") is required"));
	}
}